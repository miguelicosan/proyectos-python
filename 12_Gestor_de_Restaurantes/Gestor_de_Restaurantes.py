...
def total():
    ...
    for cantidad in texto_comida:
        sub_total_comida = round(sub_total_comida + (float(cantidad.get()) * precios_comida[p]), 2)
        p += 1

    sub_total_bebida = 0
    p = 0
    for cantidad in texto_bebida:
        sub_total_bebida = round(sub_total_bebida + (float(cantidad.get()) * precios_bebida[p]), 2)
        p += 1
...
def recibo():
    ...
    texto_recibo.insert(END, f' Precio de la Postres: \t\t\t\t{var_precio_postre.get()}\n')
    texto_recibo.insert(END, f'-' * 75 + '\n')
...
aplicacion.geometry('1184x530+0+0')
...
panel_superior = Frame(aplicacion,
                       bd=1, relief=FLAT)  # FLAT, RAISED, SUNKEN, GROOVE, RIDGE
...
# Panel de precios
panel_precios = Frame(panel_izquierdo,
                      bd=1,
                      relief=FLAT,
                      bg='azure4',
                      padx=50)
...
# Panel ticket
panel_ticket = Frame(panel_derecho,
                     bd=1,
                     relief=FLAT,
                     bg='burlywood')
panel_ticket.pack()
...
lista_bebidas = ['Agua', 'Coca-Cola', 'Fanta-Limón', 'Fanta-Naranja', 'Vino 1', 'Vino 2', 'Cerveza 1', "Cerveza 2"]
...
# Evitar que la pantalla se cierre para que no se pierdan las cuentas
aplicacion.mainloop()
